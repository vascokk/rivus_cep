...
init(QD) -> 
	...
	true -> rivus_cep_window:update(WinPid, Window, rivus_cep_aggregation:new_state());
	false -> ok
    end,
...
get_fast_aggr_result_set(Event, #query_state{window = AggWindow, window_pid = AggWinPid} = QueryState) -> 
    ...
    rivus_cep_aggregation:eval_resultset(QueryState#query_state.query_name, [Event], AggState, QueryState).

get_fast_aggr_result_set(QueryState) ->
    rivus_cep_aggregation:eval_resultset_fast_aggr(QueryState).
...
get_result_set(ResultSet, true, QueryState) ->
    rivus_cep_aggregation:eval_resultset(test_stmt, ResultSet, rivus_cep_aggregation:new_state(), QueryState).
    ...
    case ResultSet of
    	...
    	_ -> FirstRec = hd(ResultSet),
	     Result = case QueryState#query_state.query_plan#query_plan.has_aggregations of
			  ...
			  true -> rivus_cep_aggregation:eval_resultset(test_stmt, ResultSet, rivus_cep_aggregation:new_state(), QueryState)
		      end,
