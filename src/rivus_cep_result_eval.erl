...
eval_resultset_fast_aggr(QueryState) ->
  ...
  [Value || {_, Value} <- maps:to_list(AggrState#res_eval_state.result)].
...
eval_resultset(Stmt, [H | T], #res_eval_state{recno = RecNo, result = CurrentRes} = AggrState, QueryState) ->
  ...
  NewRes = maps:put(Key, ResultRecord, CurrentRes),
...
eval_result_record(Stmt, ResultRecord, Key, SelectClause, State) ->
  {ResNodes, NewState} = lists:mapfoldl(fun({EventName, EventParam}, Acc) when is_atom(EventName) andalso is_atom(EventParam) ->
    ...
    {ParamValue, Acc};
    ({Op, Left, Right}, Acc) ->
      eval_node(Stmt, {Op, Left, Right}, Key, ResultRecord, Acc);               (Node, Acc) when is_tuple(Node) ->
      eval_node(Stmt, Node, Key, ResultRecord, Acc);
    (Node, Acc) -> {Node, Acc}
...
eval_aggregation(sum, Stmt, Value, Key, #res_eval_state{aggrno = AggrNo, aggr_nodes = AggrNodes} = State) ->
  AggKey = {Stmt, Key, AggrNo},
  NewValue = maps:get(AggKey, AggrNodes, 0) + Value,
  NewAggrNodes = maps:put(AggKey, NewValue, AggrNodes),
  {NewValue, State#res_eval_state{aggrno = AggrNo + 1, aggr_nodes = NewAggrNodes}};
eval_aggregation(count, Stmt, _, Key, #res_eval_state{aggrno = AggrNo, aggr_nodes = Aggregations} = State) ->
  AggKey = {Stmt, Key, AggrNo},
  NewValue = maps:get(AggKey, Aggregations, 0) + 1,
  NewAggr = maps:put(AggKey, NewValue, Aggregations),
  {NewValue, State#res_eval_state{aggrno = AggrNo + 1, aggr_nodes = NewAggr}};
eval_aggregation(min, Stmt, Value, Key, #res_eval_state{aggrno = AggrNo, aggr_nodes = AggrNodes} = State) ->
  AggKey = {Stmt, Key, AggrNo},
  Old = maps:get(AggKey, AggrNodes, Value),
  NewValue = case Old < Value of
               true -> Old;
               false -> Value
             end,
