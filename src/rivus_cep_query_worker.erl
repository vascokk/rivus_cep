...
handle_info(Event, #state{query_type = QueryType} = State) when QueryType == simple->
    lager:debug("handle_info, query_type: simple,  Event: ~p",[Event]),

    Result = rivus_cep_query:process_event(Event, State),
    case Result of
    	nil -> nil;
    	_ -> [gproc:send({p, l, {Subscriber, result_subscribers}}, Result) || Subscriber<-State#state.subscribers]
    end,
    {noreply, State};
handle_info(Event, #state{query_type = QueryType} = State) when QueryType == pattern ->
    Result = rivus_cep_query:process_event(Event, State),
    case Result of
    	[] -> [];
    	_ -> [gproc:send({p, l, {Subscriber, result_subscribers}}, Result) || Subscriber<-State#state.subscribers]
